...
- **Витрина без запуска** (по умолчанию). Лента видна, но клик по видео не
  делает ничего: ни перехода, ни новой вкладки по среднему клику. При этом
  штатная кнопка YouTube «Смотреть позже» (часики на превью) работает — ею и
  пополняется план. Ссылки на каналы тоже живые: уход на канал это осознанный
  выбор, а не залипание.
